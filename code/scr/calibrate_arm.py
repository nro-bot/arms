import numpy as np
from utils import ImagePopup, Popup, Colors
...
def calibrate_xarm():
    ...
    # import time
    # while True:
    #     for j_id in xarm.arm_joint_ids:
    #         pos = xarm._to_pos_units(j_id, xarm.read_jpos([j_id])[0])
    #         print(xarm.get_joint_name(j_id), pos)
    #     time.sleep(1)
    #     print()
    ...
    # update config file
    try:
        data = np.load(paths.DEFAULT_XARM_CONFIG_PATH, allow_pickle=True).item()
    ...
    np.save(paths.DEFAULT_XARM_CONFIG_PATH, data)
