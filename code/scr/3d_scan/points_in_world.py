...
def main(args):
    ...
    try:
        ...
        cam_vertices, colors = c.get_pointcloud_open3d(return_raw=True)
        world_vertices = utils.coord_transform(c.cam2world, cam_vertices)

        tmp = world_vertices
        mask = np.logical_and(np.logical_and(
            np.logical_and(tmp[:, 0] >= -0.1, tmp[:, 0] <= 0.4),
            np.logical_and(tmp[:, 1] >= -0.25, tmp[:, 1] <= 0.37)),
            np.logical_and(tmp[:, 2] >= 0.0, tmp[:, 2] <= 1.)
        )

        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(world_vertices[mask])
        pcd.colors = o3d.utility.Vector3dVector(colors[mask])

        if args.show:
            o3d.visualization.draw_geometries([pcd])

        save_template = "data/hammer_c_{:d}_{:d}.pcd"

        i = 0
        while True:
            save_path = save_template.format(args.camera, i)
...
parser.add_argument("-p", "--prefix", default="hammer")
